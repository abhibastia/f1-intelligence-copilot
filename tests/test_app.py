...
class TestIndexErrorPath:
    ...
    def test_a_healthy_render_never_shows_the_error_panel(self, client, app_module, monkeypatch):
...
class TestChat:
    ...
    def test_agent_failure_returns_a_safe_message_not_a_500(self, client, app_module, monkeypatch):
...
class TestSearch:
    ...
    def test_search_failure_returns_a_safe_message_not_a_500(self, client, app_module, monkeypatch):
        def boom(*_a, **_k):
...
class TestDeleteRoutesAgainstLiveLakebase:
    ...
    def test_deleting_a_watchlist_item_through_the_route_removes_it(
        ...
        item_id = next(
            i["id"] for i in app_module.f1_broker.get_watchlist()["items"] if i["entity_ref"] == ref
        )

        resp = client.post(f"/api/watchlist/{item_id}/delete")
        assert resp.status_code == 302
        assert not any(
            i["entity_ref"] == ref for i in app_module.f1_broker.get_watchlist()["items"]
        )

    def test_deleting_a_note_through_the_route_removes_it(self, client, app_module, marker):
        ref = marker("note")
        ...
        assert resp.status_code == 302
        assert not any(n["note"] == ref for n in app_module.f1_broker.get_notes(2024)["notes"])

    def test_deleting_a_prediction_through_the_route_removes_it(self, client, app_module, lakebase):
